...
def analyze_cryptic_site(pdb_path, dcd_path, convex_selection, target_selection, density_map_dir, output_prefix):
    ...
    for frame_idx in tq(range(len(u.trajectory)), desc="Frames"):
        ...
        #protein_heavy_atoms = u.select_atoms('protein and (resid 172 to 206)')
        ...
        grid_based_volume = len(final_grids) * GRID_VOLUME
        ...
        # Calculate grid-based volume
        #grid_based_volume = len(final_grids) * GRID_VOLUME
        Grid_Based_Volumes.append(grid_based_volume)
...
# Save the results as CSV files with ',' delimiter, no headers, include frame indices
    ...
    # Save grid-based volume with frame indices
    grid_volume_data = np.column_stack((Frame_indices, Grid_Based_Volumes))
    np.savetxt(f'{output_prefix}_grid_based_volume.csv', grid_volume_data, delimiter=',', fmt='%d,%f', header='', comments='')
    ...
    # Plotting
    simulation_time = np.arange(len(Grid_Based_Volumes)) * 0.004  # Adjust time step as needed
    
    # Plot Grid-based Volume
    plt.figure()
    plt.plot(simulation_time, Grid_Based_Volumes)
    plt.xlabel('Time (ns)', fontsize=10, fontweight='bold')
    plt.ylabel('Grid-based Volume (Å$^3$)', fontsize=10, fontweight='bold')
    plt.title('Grid-based Volume Over Time', fontsize=11, fontweight='bold')
    plt.savefig(f'{output_prefix}_grid_based_volume_plot.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    analyze_cryptic_site(pdb_path, dcd_path, convex_selection, target_selection, density_map_dir, output_prefix)
